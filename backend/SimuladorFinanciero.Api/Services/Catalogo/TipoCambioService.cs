...
{
    /// <summary>Cotización USD → ARS vigente (cacheada por día en `tipo_cambio`, con fallback al último valor
    /// conocido si el BCRA no responde), junto con la fecha del registro del que salió el valor.</summary>
    Task<(decimal Valor, DateOnly Fecha)> ObtenerCotizacionUsdArsAsync(CancellationToken ct = default);
...
public sealed class TipoCambioService : ITipoCambioService
{
    ...
    public async Task<(decimal Valor, DateOnly Fecha)> ObtenerCotizacionUsdArsAsync(CancellationToken ct = default)
    {
        ...
        if (cotizacionHoy.HasValue) return (cotizacionHoy.Value, hoy);
